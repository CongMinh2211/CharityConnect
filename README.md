...
Không mở trực tiếp `frontend/index.html` bằng `file://`; Vite cần HTTP server.
...
- Assistant: `/assistant/chat`, `/assistant/capabilities`, `/assistant/role-guide`

API mới phục vụ demo quản trị rõ vai trò:

- `GET /assistant/role-guide?role=PUBLIC|DONOR|ORGANIZATION|ADMIN&path=/...` trả danh sách chức năng được phép, chức năng bị khóa và tips theo vai trò.
- `GET /transparency/diagnostics` tóm tắt toàn chuỗi, head hash, anchor gần nhất, số entry chưa neo và khuyến nghị.
- `GET /transparency/diagnostics/receipts/{receiptNumber}` kiểm tra biên nhận, proof, anchor và kết luận `CONFIRMED | UNANCHORED | INVALID`.
- `GET /transparency/diagnostics/ledger/{position}` kiểm tra một ledger entry độc lập, không lộ email/donor ID.
- `GET /transparency/anchors/{anchorId}/verify-onchain` đọc lại giao dịch Sepolia, giải mã calldata và xác nhận Merkle root khớp trên chain kèm số confirmations (anchor LOCAL_SIMULATION trả `NOT_ON_CHAIN`).
- `GET /transparency/proofs/{position}/export` xuất bundle proof tự-xác-minh (leaf, đường proof, root, anchor, hướng dẫn kiểm tra SHA-256) để bên thứ ba tự đối chiếu độc lập.
...
```bash
cd backend/identity && npm ci && npm test && npm run build
cd backend/campaign && npm ci && npm test && npm run build
cd backend/donation && pip install -r requirements.txt && pytest
...
- Vercel frontend: Root Directory `frontend`, build command `npm run build`, output `dist`, env demo `VITE_USE_MOCK_API=true`; file `frontend/vercel.json` đã có rewrite SPA cho React Router.
- Nếu deploy từ root monorepo, có thể dùng `vercel.json` ở thư mục gốc để build `frontend`.
