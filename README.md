...
- Vercel frontend: Root Directory `web`, build command `npm run build`, output `dist`, env demo `VITE_USE_MOCK_API=true`.
- Nếu deploy từ root monorepo, có thể dùng `vercel.json` hiện có để build `web`.
