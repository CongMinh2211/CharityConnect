...
pushd backend\identity
call npm run lint || goto :failed
call npm test -- --runInBand --forceExit || goto :failed
popd

echo [2/5] Campaign Service
pushd backend\campaign
call npm run lint || goto :failed
call npm test -- --runInBand --forceExit || goto :failed
popd

echo [3/5] Donation Service
"%PY%" -m pip install --disable-pip-version-check -q -r backend\donation\requirements.txt
pushd backend\donation
"%PY%" -m pytest || goto :failed
popd

echo [4/5] Assistant Service
"%PY%" -m pip install --disable-pip-version-check -q -r backend\assistant\requirements.txt
pushd backend\assistant
"%PY%" -m pytest || goto :failed
popd
