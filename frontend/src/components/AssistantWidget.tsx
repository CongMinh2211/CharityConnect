...
export function AssistantWidget(): JSX.Element {
  ...
  async function send(message: string): Promise<void> {
    ...
    const choiceIndex = /^[1-3]$/.test(clean) ? Number(clean) - 1 : -1;
    const routedMessage = choiceIndex >= 0 && suggestions[choiceIndex] ? suggestions[choiceIndex] : clean;
    ...
    try {
      const result = await askAssistant({ message: routedMessage, history, page: { path: location.pathname, role: user?.role ?? null } });
  ...
  return <div className="fixed bottom-[calc(5.75rem+env(safe-area-inset-bottom))] right-3 z-[80] sm:bottom-7 sm:right-7">
