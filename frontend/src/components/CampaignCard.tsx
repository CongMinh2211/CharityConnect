...
export function CampaignCard({ campaign }: CampaignCardProps): JSX.Element {
  ...
  return (
    <article className="campaign-card group">
      <Link className="relative block overflow-hidden" to={`/chien-dich/${campaign.id}`} aria-label={`Xem ${campaign.title}`}>
        ...
        <span className="absolute right-3 top-3 rounded-full bg-white/95 px-2.5 py-1 text-xs font-black text-brand-700 shadow-card">{percent}%</span>
      </Link>
      <div className="flex flex-1 flex-col p-5 sm:p-6">
        <div className="flex items-center justify-between gap-3 text-xs font-bold uppercase tracking-[0.12em]">
          <span className="rounded-full bg-brand-50 px-2.5 py-1 text-brand-700">{campaign.category}</span>
