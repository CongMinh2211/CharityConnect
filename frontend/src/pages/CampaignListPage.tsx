...
import { ArrowRight, FileCheck2, HeartHandshake, ReceiptText, Search, ShieldCheck, SlidersHorizontal } from "lucide-react";
...
import { FeatureHub } from "../shared/components/FeatureHub";
...
export function CampaignListPage(): JSX.Element {
  ...
  return <>
    ...
    <section className="border-y border-ink/10 bg-white"><div className="container-page grid gap-4 py-6 sm:grid-cols-3"><TrustPill icon={ShieldCheck} title="Kiểm duyệt hai lớp" text="Hồ sơ tổ chức và chiến dịch đều qua duyệt." /><TrustPill icon={ReceiptText} title="Biên nhận tức thì" text="Mỗi đóng góp có mã và mã QR riêng." /><TrustPill icon={HeartHandshake} title="Theo dõi tiến độ" text="Báo cáo quỹ và bằng chứng công khai." /></div></section>
...
function TrustPill({ icon: Icon, title, text }: { icon: typeof ShieldCheck; title: string; text: string }): JSX.Element {
  return (
    <div className="flex items-start gap-3 rounded-2xl border border-ink/10 bg-sage-100/60 p-4">
      <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-brand-500 text-ink"><Icon size={20} /></span>
      <div><p className="font-black text-ink">{title}</p><p className="mt-0.5 text-xs leading-5 text-slate-600">{text}</p></div>
    </div>
  );
}
