    ...
    <div className="container-page py-10 lg:py-16">
      <div className="mx-auto grid max-w-5xl overflow-hidden rounded-[2rem] border border-ink/10 bg-white lg:grid-cols-[.95fr_1.05fr]">
        <section className="bg-ink p-8 text-white sm:p-10">
          <p className="text-sm font-bold uppercase tracking-[.16em] text-brand-500">{isMockMode ? "Ba vai trò, một nền tảng" : "Chào mừng trở lại"}</p>
          <h1 className="mt-4 text-4xl font-black tracking-[-0.04em]">Đăng nhập để tiếp tục hành trình.</h1>
          <p className="mt-4 leading-7 text-white/65">Mỗi vai trò chỉ thấy đúng công việc của mình. Chọn nhanh một tài khoản gợi ý để tự điền thông tin đăng nhập.</p>
        ...
        <form className="p-8 sm:p-10" onSubmit={submit}>
          ...
          {isMockMode && <p className="mt-2 text-sm text-slate-500">Mật khẩu chung: <code className="rounded bg-sage-100 px-2 py-1 font-bold text-ink">Demo@123</code></p>}
