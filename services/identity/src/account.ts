...
accountRouter.post("/auth/password-reset/confirm", async (req, res, next) => {
  try {
    ...
    const passwordHash = await bcrypt.hash(input.new_password, 12);
    await query("UPDATE users SET password_hash=$1,updated_at=now() WHERE id=$2", [passwordHash, tokenRow.user_id]);
...
accountRouter.patch("/admin/users/:id/status", authenticate, authorize("ADMIN"), async (req: AuthRequest, res, next) => {
  try {
    ...
    if (input.status === "DISABLED") await query("UPDATE account_sessions SET revoked_at=now() WHERE user_id=$1 AND revoked_at IS NULL", [req.params.id]);
