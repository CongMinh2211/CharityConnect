...
describe("identity HTTP API", () => {
  ...
  it("supports account profile patch and password changes", async () => {
    ...
    expect(auditMock).toHaveBeenCalledWith(expect.any(String), "PASSWORD_CHANGED", "USER", expect.any(String), null, expect.any(Object));
  ...
  it("shows personal audit and lets admin manage user status", async () => {
    ...
    queryMock
      ...
      .mockResolvedValueOnce([] as never);
    const disabled = await request(app).patch("/admin/users/u1/status").set("Authorization", `Bearer ${adminToken}`).send({ status: "DISABLED" });
    expect(disabled.status).toBe(200);
    expect(disabled.body.status).toBe("DISABLED");

    expect((await request(app).patch("/admin/users/00000000-0000-0000-0000-000000000003/status").set("Authorization", `Bearer ${adminToken}`).send({ status: "DISABLED" })).status).toBe(409);
    queryMock.mockResolvedValueOnce([] as never);
