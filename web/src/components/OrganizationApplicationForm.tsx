...
export function OrganizationApplicationForm(): JSX.Element {
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => client.invalidateQueries({ queryKey: ["organization-profile"] })
  });
  function submit(event: FormEvent<HTMLFormElement>): void { event.preventDefault(); mutation.mutate(); }
  return <form className="card p-6" onSubmit={submit}><h2 className="text-xl font-black">Hồ sơ xác minh tổ chức</h2><p className="mt-2 text-sm text-slate-600">Chỉ cần thông tin phục vụ kiểm duyệt cốt lõi.</p><label className="mt-5 block"><span className="label">Tên pháp lý</span><input className="input" value={form.legalName} onChange={(e) => setForm({ ...form, legalName: e.target.value })} required /></label><label className="mt-4 block"><span className="label">Mã đăng ký</span><input className="input" value={form.registrationNumber} onChange={(e) => setForm({ ...form, registrationNumber: e.target.value })} required /></label><label className="mt-4 block"><span className="label">Mô tả</span><textarea className="input min-h-28" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /></label><label className="mt-4 block"><span className="label">Tài liệu xác minh</span><input className="input py-2" type="file" accept=".pdf,.png,.jpg,.jpeg" onChange={(e) => setDocument(e.target.files?.[0] ?? null)} /></label>{mutation.isError && <p className="mt-4 text-sm font-semibold text-rose-700">{mutation.error.message}</p>}<button className="btn-primary mt-5" disabled={mutation.isPending}>Nộp hồ sơ</button></form>;
}
