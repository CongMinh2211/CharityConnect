  ...
  return (
    <div>
      ...
      <div className="mt-5 grid gap-3">
        {sessions.length === 0 && <p className="rounded-2xl bg-sage-100 p-4 text-sm text-slate-500">Chưa có phiên mô phỏng. Hãy đăng xuất/đăng nhập lại để tạo phiên.</p>}
          ...
          <article key={session.id} className="rounded-2xl border border-ink/10 p-4">
            <div className="flex flex-col justify-between gap-3 sm:flex-row">
              <div>
                ...
                <p className="mt-1 break-all text-xs text-slate-500">{session.user_agent ?? "Trình duyệt demo"} · {session.ip_address ?? "local"}</p>
