...
import { AlertTriangle, ArrowRight, BarChart3, Bell, FileDown, Heart, Landmark, LayoutDashboard, ListChecks, ReceiptText, ShieldCheck, WalletCards } from "lucide-react";
      ...
      <section className="container-page pb-14">
        <div className="rounded-[2rem] border border-ink/10 bg-white p-5 shadow-card sm:p-8">
          <div className="grid gap-6 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
            <div>
              <p className="eyebrow !bg-sage-100">Không gian tài khoản</p>
              <h2 className="mt-4 text-3xl font-black tracking-[-.035em]">Đăng nhập để thấy đúng chức năng của bạn</h2>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <LoginRoleCard title="Người quyên góp" email="donor@demo.vn" text="Lưu, theo dõi, thông báo, lịch sử và PDF." />
              <LoginRoleCard title="Tổ chức" email="org@demo.vn" text="Chiến dịch, ngân sách, mốc và báo cáo quỹ." />
              <LoginRoleCard title="Admin" email="admin@demo.vn" text="Kiểm duyệt, Risk Score, Audit Log, TrustChain." />
            </div>
          </div>
        </div>
      </section>
    ...
    <section className="container-page pb-14">
      ...
      >
        ...
        <ActionGrid>
          ...
          <ActionCard icon={Landmark} title="TrustChain Anchor" text="Neo ledger vào Merkle root mô phỏng hoặc blockchain thật." to="/quan-tri?tab=trustchain" />
        </ActionGrid>
      </WorkspaceShell>
    </section>
  );
...
function LoginRoleCard({ title, email, text }: { title: string; email: string; text: string }): JSX.Element {
  return (
    <Link to="/dang-nhap" className="rounded-2xl border border-ink/10 bg-sage-100 p-4 transition hover:-translate-y-0.5 hover:border-brand-500">
      <p className="font-black">{title}</p>
      <p className="mt-1 font-mono text-xs text-brand-700">{email}</p>
      <p className="mt-3 text-sm leading-6 text-slate-600">{text}</p>
      <p className="mt-4 inline-flex items-center gap-1 text-sm font-black text-ink">Đăng nhập <ArrowRight size={15} /></p>
    </Link>
  );
}
